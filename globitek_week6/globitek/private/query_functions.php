<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if ($edit_user && !is_blank($user['password'])) {
      // confirm that previous password is correct
      $users_result = find_users_by_username($user['username']);
      // No loop, only one result
      $prev_user = db_fetch_assoc($users_result);
      if(!$prev_user || !password_verify($user['previous_password'], $prev_user['hashed_password'])) {
        ...
        $errors[] = "Password must be between 12 and 72 characters.";
      ...
      // run all validations when not editing an existing user
  ...
  function update_user($user) {
    ...
    if (!empty($errors)) {
     ...
     // hash given password (doesn't matter if password is blank here)
    $hashed_password = password_hash($user['password'], PASSWORD_BCRYPT,
      ['cost' => 11]);
    ...
    $sql .= strlen($user['password']) > 0
      ? "hashed_password='" . db_escape($db, $hashed_password) . "', "
      : "";
